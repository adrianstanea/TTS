...
class TextMelDataset(torch.utils.data.Dataset):
    ...
    def get_pair(self, filepath_and_text):
        ...
        if self.preprocess:
            ...
            mel = torch.from_numpy(np.load(os.path.join(os.path.dirname(filepath), "mel", basename)))
        else:
    ...
    def make_preprocess(self):
        ...
        if os.path.exists(base_dir):
            logging.info("Preprocessed data already exists. Skipping preprocessing.")
            return
        os.makedirs(base_dir)

        for filepath, _ in self.filepaths_and_text:
            mel = self.get_mel(filepath).cpu().detach().numpy()
            basename = os.path.basename(filepath)
            basename = os.path.splitext(basename)[0] + ".npy"
            np.save(os.path.join(base_dir, basename), mel)
...
class TextMelSpeakerDataset(torch.utils.data.Dataset):
    def __init__(self, filelist_path, cmudict_path, random_seed, add_blank=True,
